...
int main(int argc, char** argv)
{
	...
	//deviceConfig.color_resolution = K4A_COLOR_RESOLUTION_2160P; //K4A_COLOR_RESOLUTION_OFF
	//deviceConfig.synchronized_images_only = TRUE;
	deviceConfig.camera_fps = K4A_FRAMES_PER_SECOND_30; //K4A_FRAMES_PER_SECOND_30
	...
	printf(TEXT("PLEASE CONNECT THE PYTHON CLIENT!!!!!"));
	...
	if (pchRequest == NULL)
	{
		...
		printf("Client Request Strings: %s", pchRequest);
	}
	...
	if (pchReply == NULL)
	{
		...
		printf("Client Response is valid.");
	}
    ...
    while (s_isRunning)
    {
        ...
        if (popFrameResult == K4A_WAIT_RESULT_SUCCEEDED)
        {
			...
			k4a_image_t ir_image = k4a_capture_get_ir_image(originalCapture);
			//k4a_image_t colorImage = k4a_capture_get_color_image(originalCapture);


			// setup images to send to python thread
			// Write depth image data to reply
			uint8_t* depth_image_buf = k4a_image_get_buffer(depthImage);
			size_t depth_buf_size = k4a_image_get_size(depthImage);
    ...
    std::cout << "Finished EY's body tracking processing!" << std::endl;
